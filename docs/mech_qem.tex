...
\begin{itemize}
  ...
  \item \textbf{Curvature probe:} affine fields are flat, so they never bend a flat patch
        and the hinge is invisible to them. Enriching the probe with the quadratic fields
        $\mathbf{u}(X)=\tfrac12 X^\top H X+FX+\mathbf{c}$ (constant curvature) makes bending
        register. Same additive bookkeeping, a slightly larger quadric.
\end{itemize}
...
\medskip
\noindent The loop itself is ordinary greedy QEM---score every edge, collapse the cheapest,
re-score the affected ring---with the geometric face quadric swapped for this mechanical,
probe-projected one. A caveat worth keeping in view: minimizing each collapse is greedy and
path-dependent, so a locally better placement need not improve the \emph{global} coarse
response; the per-step cost is an honest local score, not a global optimum.
